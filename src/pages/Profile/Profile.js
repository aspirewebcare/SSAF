...
const Profile = () => {
  ...
  useEffect(() => {
    if (loggedUser?.jwt_token) {
      ApiRequest('GET', '/staffs/my_profile', loggedUser.jwt_token,)
        .then(result => {
      ...
      <LoadingForFetch maxDataLoad={0} totalLength={0} fetchAgain={false} fetchStatus={fetchStatus}>
        <section className="bg-white rounded-lg p-8  w-full lg:w-[1140px]">
          <div className="flex gap-8 items-center">
            ...
            <div>
              <p className="font-semibold text-2xl">{profileInfo?.first_name}</p>
              <p className="">{profileInfo?.last_name}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row flex-wrap  justify-between mt-12 gap-y-10">
            ...
            <div className="w-full lg:w-1/2 px-2">
              ...
              <input
                defaultValue={profileInfo?.manager_uid}
                placeholder="Manager"
                type="text"
                className="bg-[#F8F8F8] border rounded-lg py-3 px-3 w-full"
                name="Manager"
